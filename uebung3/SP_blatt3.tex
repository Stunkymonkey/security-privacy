...
On the other hand, we started with soundness bound $\frac{1}{3}$, which means $V$ would accept for $x\notin L$ with a probability lower than or equal to $\frac{1}{3}$. For $V'$ we can conclude that it accepts with a probability lower than or equal to $\frac{1}{3^n}$. %Note that $f(n) = \frac{1}{3^n}$ is a negligible function.
